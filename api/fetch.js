    ...
    try {
      ...
      if (r.ok) {
        try {
          ...
          // 200 but no video URL - return full response so we can see structure
          return res.json({ success: false, error: 'Got 200 but no video URL', debug: data });
        } catch(e) {
          return res.json({ success: false, error: 'Got 200 but not JSON', debug: text.substring(0, 500) });
        }
      }
    } catch(e) {
